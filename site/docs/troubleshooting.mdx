...
### Cannot unlock the vault – Wrong passphrase
...
**Rate limiting:** After several failed attempts, Tegata applies exponential backoff. The error message includes the number of remaining attempts. Wait for the indicated delay before trying again. Rate-limit state persists across sessions because it is stored in the vault header.

**Vault recovery:** If you have forgotten your passphrase, you can unlock the vault with your recovery key:
...
The recovery key is a separate 32-byte random value generated during vault initialization. It is not derived from your passphrase and cannot be used as one. If you have the recovery key but not the passphrase, run `tegata verify-recovery` to unlock the vault and then use `tegata change-passphrase` to set a new passphrase.
...
### Cannot find vault file

**Symptom:** `Error: no vault found. Use --vault, set TEGATA_VAULT, or run from the vault directory.`

Tegata searches for the vault in this order:

1. `--vault` flag
2. `TEGATA_VAULT` environment variable
3. `./vault.tegata` in the current directory

**Fix:** Specify the vault path explicitly, replacing `/path/to/vault` with the actual path to your vault:
...
This usually means either the wrong file was specified or the vault was damaged (power loss during write, filesystem error).

**Try the backup first:** Tegata creates a `.bak` file in the vault directory before every write. If `vault.tegata.bak` exists, try renaming it to `vault.tegata`:
...
### Vault locked after idle timeout
...
### Clipboard auto-clear fires too quickly (or too slowly)
...
### Code is not copied to clipboard on Linux
...
### Clipboard does not work on WSL2
...
### Ledger setup hangs at "waiting for HashStore contracts"
...
When a credential is removed with `tegata remove`, the label is no longer in the vault. Events logged before removal are preserved in the ledger but Tegata cannot resolve the label hash to a name. The history output shows `(deleted)` for these events. This is expected behavior.
