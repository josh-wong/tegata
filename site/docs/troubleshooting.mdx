...
This section describes common problems with starting the desktop GUI on each platform.

### The GUI doesn't launch

**Symptom:** The installer succeeds, but the GUI does not open or exits immediately.

<Tabs groupId="os" queryString>
  <TabItem value="windows" label="Windows">

If Windows shows "Windows protected your PC", the binary is being blocked by SmartScreen reputation checks.

1. Select **More info**.
2. Choose **Run anyway**.

This is expected for new open-source binaries that do not yet have broad reputation history.

  </TabItem>
  <TabItem value="macos" label="macOS">

If macOS reports that Tegata cannot be opened because the developer cannot be verified, allow launch via **System Settings → Privacy & Security → Open Anyway**.

If the app is still blocked, clear quarantine and retry:
...
The GUI requires a system WebKit2GTK runtime. If the app does not start, install runtime dependencies and retry:
...
If launch still fails, run from a terminal to capture loader errors and attach them to a new [issue](https://github.com/josh-wong/tegata/issues/new?assignees=&labels=triage%2C+needs+triage%2C+bug%2C+gui&template=bug_report.md&title=%5BGUI+bug%5D+Brief+description+of+the+issue):
