...
Tegata runs on Windows 10+, macOS 12+, and Linux (amd64) without any runtime dependencies. This page covers platform-specific configuration that goes beyond the [Getting started](getting-started.mdx) guide — USB drive formatting, permission issues, auto-start behavior, and known platform quirks.
...
Format your USB drive as **exFAT** for maximum cross-platform compatibility. FAT32 works but has a 4 GB file-size limit and does not support true atomic renames; Tegata handles this gracefully, but exFAT is preferable.

To format in File Explorer: right-click the drive → **Format** → set File system to **exFAT** → **Start**.

### Running the binary

PowerShell does not search the current directory for executables. Prefix the command with `.\` or add the drive root to your `PATH`.
...
```bash
xattr -d com.apple.quarantine tegata-darwin-arm64
```

If the binary is already on your USB drive:
...
:::note Code signing
The Tegata binaries are ad-hoc signed but not notarized with Apple's notarization service. The [macOS code signing doc](macos-code-signing.md) explains the current signing status and plans for full notarization.
:::
...
| Binary | Chip |
|--------|------|
| `tegata-darwin-arm64` | Apple Silicon (M1, M2, M3, M4) |
...
macOS can read and write exFAT and FAT32 natively. Format your USB drive as exFAT using Disk Utility:
...
### Setting TEGATA_VAULT persistently

Add the export to your shell profile so you do not have to type `--vault` on every command:

```bash
# For zsh (default on macOS 10.15+)
echo 'export TEGATA_VAULT=/Volumes/MyUSB' >> ~/.zshrc
source ~/.zshrc

# For bash
echo 'export TEGATA_VAULT=/Volumes/MyUSB' >> ~/.bash_profile
source ~/.bash_profile
```

### Docker for audit logging

The audit logging feature requires Docker Desktop for Mac. Install it from the [Docker website](https://www.docker.com/products/docker-desktop/). After installation, Tegata can start Docker automatically when you unlock the vault — you do not need to open Docker Desktop manually.
...
Most desktop Linux distributions auto-mount USB drives to `/media/$USER/<drive-name>`. On minimal or server systems, mount manually:
...
Replace `/dev/sdb1` with the correct device node. Use `lsblk` to identify the device:
...
Tegata's audit logging requires Docker Engine 24+ with Compose v2. Install Docker Engine following the [official Docker docs](https://docs.docker.com/engine/install/). After installation, add your user to the `docker` group so Tegata can start containers without `sudo`:
...
Format your USB drive as exFAT for cross-platform compatibility:
