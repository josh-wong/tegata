...
sidebar_position: 4
description: A walkthrough of Tegata's desktop GUI — installation, vault setup, credential management, and the audit panel.
---
...
When you open Tegata for the first time, it prompts you to locate or create a vault. If your USB drive is already connected, click **Open existing vault** and navigate to the vault file. If you have not created a vault yet, click **Create new vault**.
...
:::danger Save your recovery key before clicking Continue
...
<!-- Screenshot placeholder: main window with credential list, TOTP countdown timers, light theme -->
<!-- Image: docs/images/gui-main-light.png -->

The left sidebar lists all credentials. Each TOTP credential shows a circular countdown indicator that depletes as the current code window expires. The code refreshes automatically when the window rolls over — you do not need to click.

### Generate a code

Click any credential in the list to select it. The code and countdown appear in the detail panel on the right. Click **Copy** to copy the code to your clipboard. The clipboard is cleared automatically after 45 seconds.

For static password credentials, clicking **Copy** retrieves the password without displaying it in the UI. To display the password, click the eye icon next to the field.

### Dark and light themes

<!-- Screenshot placeholder: main window, dark theme -->
<!-- Image: docs/images/gui-main-dark.png -->
...
Click the **+** button in the toolbar or the **Add credential** option in the File menu to open the add credential dialog.

<!-- Screenshot placeholder: add credential dialog, TOTP tab selected -->
<!-- Image: docs/images/gui-add-credential.png -->

The dialog has four tabs corresponding to the four credential types: **TOTP**, **HOTP**, **Challenge-response**, and **Static password**. Fill in the required fields and click **Add**.

You can also add a credential by pasting an `otpauth://` URI directly. Click **Scan URI** and paste the URI into the input field. The type, label, issuer, algorithm, digits, and period are populated automatically.
...
Click the gear icon or open **Settings** from the application menu to access the settings panel.

<!-- Screenshot placeholder: settings panel -->
<!-- Image: docs/images/gui-settings.png -->
...
<!-- Screenshot placeholder: audit panel showing event history table -->
<!-- Image: docs/images/gui-audit-panel.png -->

The panel shows a table of recorded events with columns for operation type, credential label, timestamp (UTC), and event hash. Use the date filters to narrow the range. Click **Verify integrity** to run `tegata verify` and check the entire audit chain.

Events for removed credentials show as **(deleted)** in the label column. This is expected — the label hash is preserved in the ledger but the plaintext label is no longer in the vault.
...
For a keyboard-driven terminal experience without the overhead of a graphical application, see the [TUI guide](tui-guide.mdx).
