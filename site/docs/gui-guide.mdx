...
Select the **+** button in the toolbar to open the add credential dialog.
...
![Settings panel showing available configuration options](./images/gui-settings.png)
...
The **Vault** section of the settings panel contains options for managing your vault credentials and passphrase.

| Option | Description |
|--------|-------------|
| **Change passphrase** | Rotate your vault passphrase |
| **Verify recovery key** | Confirm that your stored recovery key is still valid |
| **Export credentials** | Create an encrypted `.tegata-backup` file with an independent export passphrase |
| **Import credentials** | Merge credentials from a `.tegata-backup` file into the current vault |

To switch to a different vault, select the **Switch vault** icon in the header toolbar. If you select an existing vault, the current vault is locked before the new one opens.

### Support the project

The **Support the project** section includes links to [GitHub Sponsors](https://github.com/sponsors/josh-wong), [Ko-fi](https://ko-fi.com/josh_haha), and [PayPal](https://www.paypal.me/joshww).

:::note[Support banner]

A banner with the same links also appears at the bottom of the main window every 10 vault unlocks. Select **X** to dismiss it for the current session, or choose **Don't show again** to hide it permanently.
