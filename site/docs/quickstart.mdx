...
The CLI and TUI runs directly from your USB drive and requires no installation on the host machine.
...
The following steps use the CLI and TUI. If you installed only the desktop GUI, see [Using the GUI](gui-guide.mdx) for the equivalent walkthrough.
...
If you prefer a visual interface in the terminal, run `tegata ui` to launch the TUI. It shows live TOTP countdown timers, a credential list you can navigate with the keyboard, and an overlay form for adding credentials. For a complete walkthrough, see the [Using the TUI](tui-guide.mdx) guide.
...
After setup, every vault unlock automatically starts the Docker stack in the background. For full details, see the [Enable audit logging](scalardl-setup.mdx) guide.
