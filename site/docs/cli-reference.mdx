...
| `--lang` | string | (auto) | Language code for CLI and TUI output: `en-us` (American English) or `ja-jp` (Japanese). The short forms `en` and `ja` are also accepted and mapped automatically. Overrides the language stored in `tegata.toml` and the system locale for a single invocation. |
...
| `language` | `en-us` / `ja-jp` | Language for CLI and TUI output. Stored in `tegata.toml` so the preference travels with the vault. |

**Examples:**

```bash title="Switch to Japanese"
tegata config set language ja-jp
```

```bash title="Switch back to English"
...
The language setting is stored in `tegata.toml` alongside other vault configuration, so it follows your vault across machines—no need to set it again on a new device.

### `tegata config show`

Display the effective configuration, including values from `tegata.toml` or their defaults. Output includes the active language.
...
For more details, see the [Enable audit logging](scalardl-setup.mdx) guide.
...
Reads the `[audit]` section from `tegata.toml` and registers the configured secret key. For configuration steps, see the [Enable audit logging](scalardl-setup.mdx) guide.
...
Launch the interactive terminal user interface. If no vault is found, the TUI starts a setup wizard. For a complete walkthrough, see the [Using the TUI](tui-guide.mdx) guide.
...
| `LANG` / `LANGUAGE` / `LC_ALL` / `LC_MESSAGES` | System locale variables. Tegata reads these to detect the preferred language when no `language` value is set in `tegata.toml` and `--lang` is not provided. The full locale tag and the language-only portion are both matched (`ja_JP.UTF-8` → `ja-jp`, `en_US.UTF-8` → `en-us`). Supported values: `en-us`, `ja-jp`. |
...
Tegata supports American English (`en-us`) and Japanese (`ja-jp`). The language is resolved in this order:

1. `--lang` flag (single command override, highest priority)
2. `language` in `tegata.toml` (stored with the vault, travels across machines)
3. System locale (`$LANG`, `$LANGUAGE`, `$LC_ALL`, `$LC_MESSAGES`)
4. English (default)

To change your default language persistently, use `tegata config set language <code>`. To override for one command, use `--lang`.
