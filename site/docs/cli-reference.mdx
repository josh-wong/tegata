...
:::note Experimental
`tegata bench` is an experimental command. Its flags and output format may change without notice.
:::
...
See the [ScalarDL setup guide](scalardl-setup.md) for full details.

:::note Experimental
Ledger commands are experimental. Their flags and behavior may change in future releases.
:::
...
Reads the `[audit]` section from `tegata.toml` and registers the configured secret key. See the [ScalarDL setup guide](scalardl-setup.md) for configuration steps.
...
Commands and their flags are classified as stable or experimental. Stable commands will not have breaking changes without a major version bump. See [STABILITY.md](https://github.com/josh-wong/tegata/blob/main/STABILITY.md) for the full list.
