...
- Error responses always include `"code"` (integer, matching exit code), `"category"` (string, error domain), and `"message"` (string, human-readable).
...
**Sidebar width:** Fixed 30 characters (28 content + 2 border characters). Labels truncate with `…` at 20 characters. The type badge is right-aligned within the sidebar column.
