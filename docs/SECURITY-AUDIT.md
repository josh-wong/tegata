...
TOTP (`internal/auth/totp.go`) and HOTP (`internal/auth/hotp.go`) implementations follow RFC 6238 and RFC 4226 respectively. The shared `computeHOTP` function in `internal/auth/otp.go` implements dynamic truncation per RFC 4226 section 5.4 using big-endian counter encoding. SHA-1 is the default HMAC algorithm per RFC specification; SHA-256 and SHA-512 are supported via `hashFuncFromAlgorithm`. SHA-1 usage in the HMAC context is not a weakness because HMAC-SHA1 security depends on the pseudorandom function properties of SHA-1, not its collision resistance.
