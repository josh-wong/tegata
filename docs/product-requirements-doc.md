...
- 1+ third-party blog posts or reviews
- Functional on all three major desktop OSes
...
- **Language:** Go or Rust (compiles to a single static binary per platform—no runtime dependencies)
- **Functionality:**
...
- **Contracts:** Will primarily use ScalarDL 3.12's generic HashStore contracts (`object.Put`, `object.Validate`) to store and verify event hashes, avoiding the need to write and deploy custom Java contracts
...
- Creates platform-specific portable binaries directory
...
- Log entries include: timestamp, operation type, credential label hash, target service hash, success/failure, host machine fingerprint hash
...
- Uses ScalarDL's built-in `object.Validate` contract
...
Tegata will use ScalarDL 3.12's **generic contracts** (HashStore) rather than custom contracts:
...
Since the Tegata host app is written in Go or Rust (not Java), the ScalarDL Java Client SDK cannot be used directly. Tegata will implement a lightweight gRPC client based on ScalarDL's protobuf service definitions. This client will support:
...
| ScalarDL gRPC client implementation may diverge from Java SDK behavior | Medium | Test against ScalarDL 3.12 integration tests. Engage with Scalar Labs community for protocol questions. |
...
- [ScalarDL Generic Contracts (HashStore) Reference](https://scalardl.scalar-labs.com/docs/latest/generic-contracts-reference/)
